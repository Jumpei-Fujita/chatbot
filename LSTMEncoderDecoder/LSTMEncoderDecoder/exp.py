...
def train_i(model, optimizer, batch_size, gpu, train_dataset, tokenizer):
    ...
    for i in lr_list:
        ...
        loss_i = criterion(output[0, :-1, :], answer[0, 1:])
...
def validation_generation(model, validation_dataset, gpu):
    ...
    print("Generated Answer : ", model.generate(question, sep, cls))
    print("~~~~")

def test_generation(model, test_dataset, gpu, data_num, tmp=0.9, top_p=0, top_k=40):
    if gpu:
        model = model.cuda()
    else:
        model = model.cpu()
    ...
    print("question : ", tokenizer.decode(question[0].cpu().numpy()))
    print("gold answer : ", model.word_decode(answer[:, :-1]))
    print("Generated Answer : ", model.generate2(question, sep, cls, tmp, top_p, top_k))

def QandA(model, question, gpu, show_Q, tokenizer=tokenizer):
    if show_Q:
        print("Q : ", question)
    question = make_question(question, tokenizer)
    sep = torch.LongTensor([tokenizer.sep_token_id]).unsqueeze(0)
    ...
    if gpu:
        ...
        model = model.cpu()
    with torch.no_grad():
        answ = model.generate2(question, sep, cls)
        print("A : ", answ)
    return answ
